...
# establish multi plot device
library(datasets)
par(mfrow = c(2, 2))

# Draw plots on screen device
# first plot
with(myHPC,plot(DiffTime,Global_active_power,xlab="",type="l",ylab = "Global Active Power",xaxt='n') )
...
# second plot
with(myHPC,plot(DiffTime,Voltage,type="l",ylab = "Voltage",xlab = "datetime",xaxt='n') )
#Lable axis x as per task
# weekdays(as.Date(myHPC[,"Date"],format ="%d/%m/%Y")) - find week days of the range  result - Thursday and Friday, so labels should be set accordingly
# In the task there are only three ticks on axes x so there should be only min, avg and max DiffTime
axis(side=1,at=c(0.0,1.0,2.0),labels=c("Thu","Fri","Sat"))
...
axis(side=1,at=c(0.0,1.0,2.0),labels=c("Thu","Fri","Sat"))
#legend
legend("topright",inset = 0.04, lty=1, box.lty=0,  cex=0.7, text.width = 0.8, col = c("black", "red","blue"), legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"))

# forth plot
with(myHPC,plot(DiffTime,Global_reactive_power,xlab = "datetime",type="l",xaxt='n') )
#Lable axis x as per task
# weekdays(as.Date(myHPC[,"Date"],format ="%d/%m/%Y")) - find week days of the range  result - Thursday and Friday, so labels should be set accordingly
# In the task there are only three ticks on axes x so there should be only min, avg and max DiffTime
axis(side=1,at=c(0.0,1.0,2.0),labels=c("Thu","Fri","Sat"))
...
## Copy multiplot window from screen to file device
